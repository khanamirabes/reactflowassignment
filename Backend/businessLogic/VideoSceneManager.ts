...
export class VideoSceneManager {
  ...
  public static updateNodePosition(request: any, response: any) {
    const body = request.body
    const id = body.id;
    const postion = body.position
    db.VideoScene.update({ postion: postion }, {
      where: { id: id }
    }).then((result: any) => {
      console.log('this is updateNodePosition', result)
      response.status(200)
      response.json(result)
      return;
    }).catch((err: any) => {
      console.log('this is err', err, body)
    })
